...
Open WebUI connects to Ollama on the private Docker network. It trusts the Authelia identity headers from the Caddy proxy: the first user who signs in through SSO is provisioned as admin automatically, and users whose LLDAP email matches an existing Open WebUI account are signed straight in. It has no direct port — the proxy is the only way in.
