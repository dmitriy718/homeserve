...
- SSO portal: `https://auth.homeserve.lan` — log in as `admin` with the
  `LLDAP_LDAP_USER_PASS` value from `/srv/ai-node/secrets/lldap.env`
  (generated by the installer). Every protected UI redirects here on first
