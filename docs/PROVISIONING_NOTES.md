...
- The first final reboot exposed a Docker/Wi-Fi ordering race: port-bound containers attempted to start before `192.168.1.68` existed and stayed exited. `ai-node-stack.service` now waits for both the LAN and Tailscale addresses and runs Compose after networking; the reboot persistence test was repeated after this correction.
